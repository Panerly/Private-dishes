...
- (void)setScrollViewContentSize {
    if (_images.count > 1) {
        ...
        self.currImageView.frame = CGRectMake(kScreenWidth * 2, 0, kScreenWidth, 180);
        
        if (_changeMode == ChangeModeFade) {
            //淡入淡出模式，两个imageView都在同一位置，改变透明度就可以了
            _currImageView.frame = CGRectMake(0, 0, kScreenWidth, 180);
        ...
        self.currImageView.frame = CGRectMake(0, 0, kScreenWidth, 180);
        [self stopTimer];
    }
}
...
- (void)setPagePosition:(PageControlPosition)pagePosition {
    ...
    CGFloat centerY = 180 - size.height * 0.5 - VERMARGIN - (_describeLabel.hidden?0: DES_LABEL_H);
    CGFloat pointY = 180 - size.height - VERMARGIN - (_describeLabel.hidden?0: DES_LABEL_H);
    
    if (_pagePosition == PositionDefault || _pagePosition == PositionBottomCenter)
        _pageControl.center = CGPointMake(kScreenWidth * 0.5, centerY);
    else if (_pagePosition == PositionTopCenter)
        _pageControl.center = CGPointMake(kScreenWidth * 0.5, size.height * 0.5 + VERMARGIN);
    else if (_pagePosition == PositionBottomLeft)
        _pageControl.frame = CGRectMake(HORMARGIN, pointY, size.width, size.height);
    else
        _pageControl.frame = CGRectMake(kScreenWidth - HORMARGIN - size.width, pointY, size.width, size.height);
}
...
- (void)nextPage {
    ...
    } else [self.scrollView setContentOffset:CGPointMake(kScreenWidth * 3, 0) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    _describeLabel.frame = CGRectMake(0, 180 - DES_LABEL_H, kScreenWidth, DES_LABEL_H);
    //重新计算pageControl的位置
    self.pagePosition = self.pagePosition;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (offsetX < kScreenWidth * 2) {
        if (_changeMode == ChangeModeFade) {
            self.currImageView.alpha = offsetX / kScreenWidth - 1;
            self.otherImageView.alpha = 2 - offsetX / kScreenWidth;
        } else self.otherImageView.frame = CGRectMake(kScreenWidth, 0, kScreenWidth, 180);
        
        self.nextIndex = self.currIndex - 1;
        if (self.nextIndex < 0) self.nextIndex = _images.count - 1;
        self.otherImageView.image = self.images[self.nextIndex];
        if (offsetX <= kScreenWidth) [self changeToNext];
        
    //向左滚动
    } else if (offsetX > kScreenWidth * 2){
        if (_changeMode == ChangeModeFade) {
            self.otherImageView.alpha = offsetX / kScreenWidth - 2;
            self.currImageView.alpha = 3 - offsetX / kScreenWidth;
        } else self.otherImageView.frame = CGRectMake(CGRectGetMaxX(_currImageView.frame), 0, kScreenWidth, 180);
        
        self.nextIndex = (self.currIndex + 1) % _images.count;
        self.otherImageView.image = self.images[self.nextIndex];
        if (offsetX >= kScreenWidth * 3) [self changeToNext];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (currPointInSelf.x >= -kScreenWidth / 2 && currPointInSelf.x <= kScreenWidth / 2)
